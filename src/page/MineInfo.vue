<template>
  <div>
    ...
    <div class="mine-info-container">
      <div class="mine-info-row">
        <div class="hhxh-flex">
          ...
          <input type="text" class="flex-1 text-right" placeholder="手机号码" readonly="readonly" v-model="data.number">
        </div>
      </div>
    </div>
    <div class="mine-info-container">
    ...
    <div class="mine-info-container">
      ...
      <div class="mine-info-row select-row active-btn" @click="open('datePicker')">
        <div class="hhxh-flex">
    ...
    <div class="mine-info-submit text-center">
      <button class="active-btn" @click="infoSave">保存</button>
    </div>
  ...
  methods: {
    ...
    infoSave: function () {
      if (this.data.name.trim() === '') {
        ...
        Toast('手机号不正确')
      } else if (this.data.gender.toString().trim() === '') {
        Toast('请选择性别')
        ...
        Toast('身份证号码格式不正确')
      } else if (this.data.socialSecurityNo.trim() !== '' && !validators.isMumOrLetter(this.data.socialSecurityNo)) {
        Toast('社保号不正确')
      } else {
        this.doAdd()
      }
      // else if (this.data.area.toString().trim() === '') {
      //   Toast('请选择所在地区')
      // }
    },
/*
    ...
    loadMineInfo: function () {
      mineInfoService.loadMineInfo().then(res => {
        ...
        this.data.birthday = res.t.birthtime || ''
    ...
    handleChange: function (value) {
      // var base = new Date(value)
      // var year = base.getFullYear()
      // var month = base.getMonth() + 1
      // var date = base.getDate()
      // this.data.birthday = year + '.' + month + '.' + date
